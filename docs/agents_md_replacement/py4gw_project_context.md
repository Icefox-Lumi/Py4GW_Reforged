...
`project_specific_context_recovered.md` preserves the previous tracked
...
- `docs/persistence/` and `docs/persistence_jail/` contain the migration,
  audit, and rationale records for this boundary.

## Documentation authority map

- `docs/architecture/conceptual_model/Py4GW_Conceptual_Model.md` is the canonical architecture and
  terminology reference.
- `docs/integration/bridge/MCP_bridge.md` is the MCP-facing planning summary; use
  `BridgeRuntime/README.md` for daemon, injected client, CLI, and operator
  usage.
- `docs/architecture/conceptual_model/Py4GW_Model_Features_Detail.txt` is a derived quick-scan export, not a
  separate authority.
- `docs/ui/widget_manager/widget_manager_and_catalog.md` is the high-value reference before
  changing widget discovery, metadata defaults, `WidgetHandler`, or
  `WidgetCatalog`.
- `docs/migration_to_reforged/` records the ongoing legacy-GWCA to
...
- Read subsystem handovers before changes in their area, including
  ...
  `ui/widget_manager/widget_manager_and_catalog.md`, `automation/behavior_trees/bottingtree_and_bt_routines_guide.md`,
  `heroai_combat_handover.md`, and `settings_database_cache_model.md`.
- The documentation index at `docs/DOCUMENTATION_INDEX.md` is a navigation aid;
...
- Start with `docs/RE/reverse_engineering_reference.md`, then use
  `docs/RE/CPP_WASM_MAPPING.md` for translation procedure and
  `docs/RE/rosetta_stone.txt` for GwA2/AutoIt mappings.
...
- `docs/RE/native_gw_ui_function_catalog.json` catalogs native UI functions;
  `docs/RE/native_gw_window_creation_investigation.md` covers window creation;
  and `docs/RE/native_ui_title_and_encoded_string_reference.md` covers title
  and encoded-string handling.
- `docs/RE/name_tag_color/Feature_Guide.md` is the historical usage guide for
  `PyAgentTagColor` (Python API, ARGB format, and in-client validation).
  `docs/RE/name_tag_color/Reverse_Engineering.md` records the native name-tag
...
- Do not commit local runtime/config churn such as `Py4GW.ini`,
  `Py4GW_Launcher.ini`, or `Py4GW_injection_log.txt` unless the task explicitly
