"""
format_catalogs.py — turn the raw Ghidra dump (raw-item-catalogs.json) into clean per-catalog
...
Run:  python docs/items/modifiers/tools/format_catalogs.py
"""
import csv
import json
import os
...
STRUCT_FIELDS = {
    ...
    # formulas: emitted from formulas-recipes.json (crafting recipes) below.
}
...
def _load_mod_names():
    """idx -> (name, description) from the already-generated game-mod-table-named.txt
    (fixed-width columns written by the Dump Named Mod Table widget)."""
    path = os.path.join(HERE, "game-mod-table-named.txt")
    out = {}
    if not os.path.exists(path):
        return out
