...
- **`ImGui_Legacy` is being retired** (see `docs/ui/imgui/ImGui_Facade_Migration_Plan.md`). v1 is built entirely on it; v2 still leaks it (`agent_demo.py`, `tooltip()`). The replacement should target the new `Py4GWCoreLib.ImGui` facade with **no bridging** to legacy (project hard rule).
