...
> Historical-source note (reviewed 2026-08-04): this guide describes the
> earlier `PyAgentTagColor` surface. The current source/stub/test path is
> `AgentRecolor`/`PyAgentRecolor`, which extends the feature to gadgets and
> ground items. Inspect `Py4GWCoreLib/AgentRecolor.py` before implementing the
> legacy API shown below.

Recolor agent overhead **name tags** (players, NPCs, enemies) — and the shared
consider/target ring — **using the game's own renderer**. No ImGui_Legacy/overlay
...
- **Reverse-engineering / internals:** `Reverse_Engineering.md`.
