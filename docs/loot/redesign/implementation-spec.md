...
From `legacy/implementation-audit-vs-plan.md`, every one of these was a decision the plan did not cover and I took
anyway:
...
**This supersedes an earlier decision in the plan.** `class.md` recorded that the display-mode
...
> **This supersedes the plan.** `class.md` has `mods_all` / `mods_any` as two separate lists on a
...
`docs/architecture/records/pending-fixes.md` PF-4.
...
> **This supersedes the plan.** `class.md` settled *"top tabs, one category per tab"* for the quick
...
Rewritten from `class.md`'s build order, with the missing part added: **each step states its
