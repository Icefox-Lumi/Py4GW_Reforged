...
Status: **historical plan; implementation exists in the current tree.** This document consolidates the locked UI
...
Current-source note (reviewed 2026-08-04):
`Py4GWCoreLib/py4gwcorelib_src/launch_bar/` is present and is hosted by
`Py4GW_widget_manager.py`. The original planning and "not yet implemented"
wording is retained as historical design context; inspect the current package
for actual behavior and completion status.
