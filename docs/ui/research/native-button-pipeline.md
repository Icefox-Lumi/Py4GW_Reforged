...
> `native-ui-controls-handover.md` — it is the source of truth. The recipes here describe the intended
...
**Code locations:** C++ `../Py4GW/include/py_ui.h` (+ `src/py_ui.cpp` bindings); Python API
...
Full model + all 166 discovered control procs: `ui-controls-master-catalog.md`.
...
content, and its clicks come from the engine's own button state — no ImGui_Legacy, no hit-test.
...
`docs/ui/research/ui-elements-creation-recipes.md`. **The BUTTON is done.**
...
**All controls:** see **`docs/ui/research/ui-elements-creation-recipes.md`** — a per-element
