...
> `native-button-pipeline.md` → "MASTER REFERENCE"**. Deltas discovered during implementation that
...
6. Docs: in docs/archive/ui/research/ui-controls-catalog.md and ui-elements-creation-recipes.md, replace the 'not a button' verdict for CtlTextBtnProc with 'clickable, notify-8 only, NO pollable pushed state, msg 0x59==GetText; requires a selectable-list or dialog-subclass parent'.
...
4) Docs: correct native-button-pipeline.md 5.2 and ../../archive/ui/research/ui-controls-catalog.md — the 06-14 CtlBtnProc HAS case 0x38 (FUN_00610580) and case 0x13 preferred-size; the width problem is the frame list, not a missing size handler. Do NOT route checkbox/radio/slider/dropdown/styled-button through the generic item creator; each needs its own recipe (separate work item).
...
3) Do NOT add AddEditableTextItemToFrameListByFrameId (the single-proc item builder in ui-elements-creation-recipes.md sec.12) — structurally wrong for edit. Editable text must use the CreateUIComponent path as a CreateNativeWindow child, not a frame-list item.
...
(3) REAL defect B - message IDs: docs/archive/ui/research/ui-controls-catalog.md (~lines 162-169) swaps 0x57/0x58. Decompile proves 0x57=GetText, 0x58=SetIsOpen.
...
5. DOCS: fix docs/archive/ui/research/ui-controls-catalog.md GroupHeader table (0x57=GetText, 0x58=SetIsOpen); note child1=CtlTextBtnProc (callback 0x0A56 / EXE FUN_00616c00) and child0=checkbox (callback 0x0AFD / EXE FUN_008867f0, self-managed image list DAT_01081d70). Mirror the swap fix + child-proc notes into native-button-pipeline.md and ui-elements-creation-recipes.md.
