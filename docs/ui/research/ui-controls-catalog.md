...
> in-client. **`native-button-pipeline.md` is now the authoritative creation reference** (master
...
> `CtlPageProc 0x0061a950`. See `native-button-pipeline.md` for the working recipes.
...
Three FrameProc candidates tested for native button creation via `CreateUIComponent`. All crash. See `docs/game-client/research/reverse-engineering-reference.md` §14.2 for full details.
