...
Workflow:
  ...
  5. Results AUTO-SAVE to UI_RE/ui-test-results.txt on every verdict + every create, so a crash never
...
RESULTS_PATH = os.path.join(_BASE, "ui-test-results.txt")
LOG_PATH = os.path.join(_BASE, "ui-test-log.txt")
...
def main() -> None:
    ...
    PyImGui.text_colored("Create -> interact -> mark verdict. Auto-saves to ui-test-results.txt.", (0.4, 0.8, 1.0, 1.0))
    done = sum(1 for v in _verdict.values() if v in VERDICTS)
    PyImGui.text(f"marked {done}/{len(CONTROLS)}   file: UI_RE/ui-test-results.txt")
    PyImGui.separator()
