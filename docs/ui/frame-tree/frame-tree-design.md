...
Status: agreed requirements; the original status is historical and must be
checked against the current `Py4GWCoreLib/FrameTree/` implementation.
Source review on 2026-08-04 found the FrameTree package present, so the old
"not yet implemented" wording is stale for the current tree. The design,
registry decisions, and deferred verification notes remain useful as the
migration record.
Date: 2026-07-29
Background RE: `../Py4GW_Reforged_Native/docs/game-client/research/ui_frame_identity_reverse_engineering.md`
...
| 4 | snapshot + query engine, unused | `RE/UI_RE/runtime_frame_tree_engine.py` |
...
Verified across the tree, excluding the package and `RE/UI_RE/`:
