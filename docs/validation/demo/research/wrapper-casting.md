...
Companion docs (read alongside): `docs/validation/demo/contexts.md` (context struct
inventory & layouts) and `docs/validation/demo/python-wrapper-api.md` (per-domain public API
...
- **Not in `GWContext` (raw-only):** `GameContext` (no facade — manual
  ...
  `GameContext + 0x18`). See `contexts.md §GameContext / §TextParser`.
...
`contexts.md`** (do not duplicate here). The casting-relevant summary:
...
- Many combat/stance/target getters are **stubbed** (hard-return `0/False/[]` pending the
  CombatEvents migration) — see `python-wrapper-api.md §Agent`. The tool should mark these
...
(`GetAllyArray/GetEnemyArray/GetItemAgentArray/…`, see `contexts.md §AgentContext`).
