...
class InventorySettingsController:
    ...
    def draw_context_menu_items(self, prepend_separator: bool = True) -> bool:
        ...
        profile = self.active()
        has_entries = profile.context_menu_xunlai or profile.colorize.context_menu_toggle
        if not has_entries:
            return False
    ...
    def _draw_context_menu(self) -> None:
        ...
        The two migrated actions need no selected item, so this callback does not scan inventory.
        ...
        profile = self.active()
        if not (profile.context_menu_xunlai or profile.colorize.context_menu_toggle):
            return
