...
def MerchantItems(index: int, message: SharedMessageStruct):
    ...
    if mode == "report_salvage_kits":
        ...
        # the calling platform (see docs/archive/persistence/ini-manager/configparser-to-settings-migration-plan.md).
        try:
...
def _queue_merchant_rules_message(index: int, message: SharedMessageStruct):
    ...
    try:
        ...
        close = getattr(routine, "close", None)
        if callable(close):
            try:
                close()
            except BaseException:
                pass
        elif dispatch_lease_acquired:
