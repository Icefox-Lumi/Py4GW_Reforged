...
class LootEx:
    ...
    def handle_keys(self):
        if not LootEx.is_left_mouse_down() or not Console.is_window_active() or PyImGui.is_any_item_hovered() or self.ui.py_io.want_capture_mouse:  # type: ignore[attr-defined]
            return
        ...
        if self.ui.py_io.key_ctrl:        
            if new_hovered_item == hovered_item:            
                ...
                if self.hotkey_timer.IsExpired():
                    ...
                    if Frame(FrameId.TradeWindow).exists:
                        ConsoleLog(MODULE_NAME, f"Offering item '{item.name}' in trade window.", Console.MessageType.Info)
                        PyTrade.offer_item(item_id=item.id, quantity=item.quantity)
                        return
    ...
    def main(self):
        ...
        if self.settings.parent_frame_id is None or self.settings.parent_frame_id == 0:
            ...
            self.settings.parent_frame_id = inventory_frame.frame_id if inventory_frame.exists else 0
