...
from Py4GWCoreLib.modular.hero_team_manager import HERO_ID_TO_INDEX
from Py4GWCoreLib.modular.hero_team_manager import HERO_IDS
from Py4GWCoreLib.modular.hero_team_manager import HERO_SLOT_COUNT
...
from Py4GWCoreLib.modular.hero_team_manager import current_party_hero_targets
from Py4GWCoreLib.modular.hero_team_manager import current_party_hero_targets_for_template
from Py4GWCoreLib.modular.hero_team_manager import dedupe_team_slots
from Py4GWCoreLib.modular.hero_team_manager import delete_team
from Py4GWCoreLib.modular.hero_team_manager import delete_template
from Py4GWCoreLib.modular.hero_team_manager import duplicate_team
from Py4GWCoreLib.modular.hero_team_manager import ensure_team_slots
from Py4GWCoreLib.modular.hero_team_manager import get_team
from Py4GWCoreLib.modular.hero_team_manager import get_template
from Py4GWCoreLib.modular.hero_team_manager import hero_alias
from Py4GWCoreLib.modular.hero_team_manager import hero_default_name
from Py4GWCoreLib.modular.hero_team_manager import hero_display_name
from Py4GWCoreLib.modular.hero_team_manager import hero_labels
from Py4GWCoreLib.modular.hero_team_manager import load_config
...
from Py4GWCoreLib.modular.hero_team_manager import save_global_templates
from Py4GWCoreLib.modular.hero_team_manager import save_template_name
...
MODULE_ICON = 'Assets\\Textures\\Module_Icons\\Hero Team Manager.png'
...
# Seed the width before BeginTooltip so wrapped preview text is not measured against
# ImGui's provisional 20px tooltip width on the first frame.
TEMPLATE_PREVIEW_TOOLTIP_WIDTH = 258.0
...
def _log_error(exc: Exception) -> None:
    PySystem.Console.Log(MODULE_NAME, f'{exc}', PySystem.Console.MessageType.Error)
    PySystem.Console.Log(MODULE_NAME, traceback.format_exc(), PySystem.Console.MessageType.Error)


def _account_editable_state(config: HeroTeamConfig) -> dict[str, object]:
    state = account_config_to_dict(config)
    state.pop('active_team_id', None)
    state.pop('hero_profession_cache', None)
    return state


def _global_editable_state(config: HeroTeamConfig) -> list[HeroTemplateEntry]:
    return [
        HeroTemplateEntry(template_id=template.template_id, name=template.name, code=template.code)
        for template in config.templates
    ]


def _capture_saved_account_state(config: HeroTeamConfig) -> None:
    global _saved_account_state
    _saved_account_state = _account_editable_state(config)


def _capture_saved_global_state(config: HeroTeamConfig) -> None:
    global _saved_global_state
    _saved_global_state = _global_editable_state(config)


def _capture_saved_editable_state(config: HeroTeamConfig) -> None:
    _capture_saved_account_state(config)
    _capture_saved_global_state(config)


def _is_account_state_dirty(config: HeroTeamConfig) -> bool:
    global _saved_account_state
    current_state = _account_editable_state(config)
    if _saved_account_state is None:
        _saved_account_state = current_state
        return False
    return current_state != _saved_account_state


def _is_global_state_dirty(config: HeroTeamConfig) -> bool:
    global _saved_global_state
    current_state = _global_editable_state(config)
    if _saved_global_state is None:
        _saved_global_state = current_state
        return False
    current_records = [(template.template_id, template.name, template.code) for template in current_state]
    saved_records = [(template.template_id, template.name, template.code) for template in _saved_global_state]
    return current_records != saved_records
...
def _sync_dirty_state(
    ...
    *,
    ...
    account_dirty = _is_account_state_dirty(current_config)
    global_dirty = _is_global_state_dirty(current_config)
    _dirty = account_dirty or global_dirty
    if update_status:
        if _dirty:
...
def _account_dirty(config: HeroTeamConfig) -> bool:
    return _is_account_state_dirty(config)


def _global_dirty(config: HeroTeamConfig) -> bool:
    return _is_global_state_dirty(config)


def _ensure_config() -> HeroTeamConfig:
    global _config, _dirty, _status, _saved_account_state, _saved_global_state
    current_key = safe_account_key()
    if _config is None:
        _config = load_config(current_key)
        _capture_saved_editable_state(_config)
        _dirty = False
    elif (_config.account_key or 'default') != current_key:
        old_global_dirty = _global_dirty(_config)
        old_global_templates = list(_config.templates) if old_global_dirty else []
        old_account_dirty = _account_dirty(_config)
        new_config = load_config(current_key)
        remote_global_templates = list(new_config.templates)
        _config = new_config
        if old_global_dirty:
            _config.templates = old_global_templates
        _capture_saved_account_state(_config)
        if old_global_dirty:
            _saved_global_state = [
                HeroTemplateEntry(template_id=template.template_id, name=template.name, code=template.code)
                for template in remote_global_templates
            ]
        else:
            _capture_saved_global_state(_config)
        _dirty = bool(old_global_dirty)
        if old_account_dirty:
            _status = 'Account changed; unsaved account edits were discarded to prevent cross-account writes.'
    return _config
...
def _save_status(message: str = 'Saved.') -> None:
    ...
    account_dirty = _account_dirty(config)
    global_dirty = _global_dirty(config)
    global_conflicts: list[str] = []

    if global_dirty:
        base_templates = _saved_global_state or []
        remote_templates = reload_global_templates()
        merged_templates, global_conflicts = merge_template_libraries(
            base_templates,
            config.templates,
            remote_templates,
        )
        if not global_conflicts:
            config.templates = merged_templates
            base_ids = {template.template_id for template in base_templates}
            merged_ids = {template.template_id for template in merged_templates}
            remote_ids = {template.template_id for template in remote_templates}
            deleted_ids = (base_ids & remote_ids) - merged_ids
            save_global_templates(merged_templates, deleted_template_ids=deleted_ids)
            _capture_saved_global_state(config)
    else:
        remote_templates = reload_global_templates()
        remote_records = [(template.template_id, template.name, template.code) for template in remote_templates]
        current_records = [(template.template_id, template.name, template.code) for template in config.templates]
        if remote_records != current_records:
            config.templates = remote_templates
        _capture_saved_global_state(config)
    ...
    _sync_dirty_state(config)
    if global_conflicts:
        _status = f'Shared template conflict; reload before saving: {", ".join(global_conflicts)}.'
    else:
        _dirty = False
        _status = message
...
def _clear_account_dirty(message: str = 'Saved.') -> None:
    global _dirty, _status
    config = _ensure_config()
    _capture_saved_account_state(config)
    _sync_dirty_state(config)
...
def _clear_global_dirty(message: str = 'Saved.') -> None:
    global _dirty, _status
    config = _ensure_config()
    _capture_saved_global_state(config)
    _sync_dirty_state(config)
...
def _mark_template_name_saved(config: HeroTeamConfig, template_id: str) -> None:
    global _saved_global_state, _dirty, _status
    template = get_template(config, template_id)
    if template is None:
        return
    saved = list(_saved_global_state or [])
    for index, saved_template in enumerate(saved):
        if saved_template.template_id == template.template_id:
            saved[index] = HeroTemplateEntry(
                template_id=saved_template.template_id,
                name=template.name,
                code=saved_template.code,
            )
            break
    else:
        saved.append(HeroTemplateEntry(template_id=template.template_id, name=template.name, code=''))
    _saved_global_state = saved
    _sync_dirty_state(config)
    _status = 'Shared template renamed and saved.'


def _reload_saved_config() -> None:
    global _config, _selected_template_id
    current = _ensure_config()
    global_templates = list(current.templates) if _global_dirty(current) else None
    _config = load_config(safe_account_key())
    if global_templates is not None:
        _config.templates = global_templates
    _selected_template_id = ''
    _clear_rename()
    _clear_code_edit()
    _capture_saved_account_state(_config)
    if global_templates is None:
        _capture_saved_global_state(_config)
    _sync_dirty_state(_config)
    _status = 'Reloaded saved account configuration.'


def _reload_shared_templates(*, discard_unsaved: bool = False) -> None:
    global _selected_template_id
    config = _ensure_config()
    if _global_dirty(config) and not discard_unsaved:
        _request_confirmation(
            'reload_shared_templates',
            'Reload Shared Templates',
            'Discard unsaved shared-template edits and reload the global library?',
        )
        return
    config.templates = reload_global_templates()
    if _selected_template_id and get_template(config, _selected_template_id) is None:
        _selected_template_id = ''
    _clear_code_edit()
    _clear_rename()
    _clear_global_dirty('Reloaded shared templates.')
...
def _button_tint(
    color: tuple[float, float, float, float], alpha: float, lift: float = 0.0
...
def _padded_tinted_text(
    text: str, color: tuple[float, float, float, float], pad: float = EMPTY_CONTROL_TEXT_PAD
) -> None:
    try:
...
def _draw_template_code_control(
    ...
    if not _is_editing_code(template_id):
        ...
        _set_cursor_pos_y(row_y)
        if _button_disabled(
            f'Edit##edit_template_code_{template_id}', disabled, CODE_EDIT_BUTTON_WIDTH, CODE_EDIT_BUTTON_HEIGHT
        ):
            _begin_code_edit(template_id, original_code)
        _show_item_tooltip('Team load is running.' if disabled else 'Edit code')
        _same_line(4)
...
def _draw_template_preview(
    ...
    title: str = '',
    compact: bool = False,
    allow_skill_tooltips: bool = True,
) -> None:
    ...
    if icons:
        preview_key = str(title or preview.template_name or 'template').replace('#', '_').replace('\n', '_')
        for icon_index, (skill_id, icon_path, skill_name) in enumerate(icons):
            if icon_index > 0:
                _same_line(2)
            ImGui.invisible_button(
                f'##hero_team_preview_skill_{preview_key}_{icon_index}_{int(skill_id)}',
                TEMPLATE_PREVIEW_ICON_SIZE,
                TEMPLATE_PREVIEW_ICON_SIZE,
            )
            item_min = PyImGui.get_item_rect_min()
            icon_hovered = bool(PyImGui.is_item_hovered()) if allow_skill_tooltips else False
            if allow_skill_tooltips and icon_hovered and PyImGui.begin_tooltip():
                try:
            ...
            ImGui.DrawTextureInDrawList(
                ...
                size=(TEMPLATE_PREVIEW_ICON_SIZE, TEMPLATE_PREVIEW_ICON_SIZE),
...
def _draw_template_preview_or_fallback(
    template,
    template_name: str = '',
    title: str = '',
    compact: bool = False,
    ...
    if preview is None:
        ...
        return
    _draw_template_preview(
        preview,
        title=title,
        compact=compact,
        allow_skill_tooltips=allow_skill_tooltips,
    )
    ...
    try:
        ...
        try:
            PyImGui.set_next_window_size(
                (TEMPLATE_PREVIEW_TOOLTIP_WIDTH, 0.0),
                PyImGui.ImGuiCond.Always,
            )
        except Exception:
            pass
        return bool(PyImGui.begin_tooltip())
    except Exception:
        return False
...
def _show_template_tooltip(template) -> None:
    ...
    try:
        if template is None:
            PyImGui.text('<None>')
            return
        _draw_template_preview_or_fallback(template, allow_skill_tooltips=False)
    finally:
        _end_tooltip()
...
def _collapsing_header_colored(
    label: str, color: tuple[float, float, float, float], default_open: bool = False
) -> bool:
    try:
        PyImGui.push_style_color(PyImGui.ImGuiCol.Text, color)
        pushed = True
...
def _load_template_group_order_once() -> None:
    ...
    settings = Settings(_window_ini_key, 'account')
    raw_order = settings.get_str(
...
def _default_template_group_order(
    ordered_groups: list[tuple[TemplateProfessionGroup, list[HeroTemplateEntry]]],
) -> list[str]:
    order: list[str] = []
...
def _visible_template_group_order(
    ordered_groups: list[tuple[TemplateProfessionGroup, list[HeroTemplateEntry]]],
) -> list[str]:
...
def _save_template_group_order(visible_order: list[str]) -> None:
    ...
    if _init_window_persistence_once():
        Settings(_window_ini_key, 'account').set_str(
            TEMPLATE_GROUP_ORDER_SECTION,
            TEMPLATE_GROUP_ORDER_KEY,
...
def _draw_template_group_selector(config: HeroTeamConfig) -> bool:
    ...
    groups: dict[str, tuple[TemplateProfessionGroup, list[HeroTemplateEntry]]] = {}
    for template in config.templates:
    ...
    groups_by_key = {
        str(getattr(group, 'group_key', '') or ''): (group, templates) for group, templates in ordered_groups
    }
...
def _draw_template_apply_control(config: HeroTeamConfig, template, stacked: bool = False) -> None:
    ...
    if _selected_apply_target_hero_id not in target_ids:
        preferred_id = template_preferred_hero_id(config, template.template_id)
    ...
    tooltip = (
        'Team load is running.' if running else 'Apply selected saved template to the selected current party hero.'
    )
    ...
    try:
        ...
        for slot_index, slot in enumerate(team.slots[:HERO_SLOT_COUNT]):
            ...
            if not template_code:
                ...
                if slot.template_id and get_template(config, slot.template_id) is None:
                    PyImGui.text(f'Missing template: {str(slot.template_id)[:18]}')
                else:
            ...
            if template is not None and not str(slot.template_code or '').strip():
                _draw_template_preview_or_fallback(
                    template,
                    title=title,
                    compact=True,
                    allow_skill_tooltips=False,
                )
            else:
                _draw_template_preview_or_fallback(
                    template_code,
...
def _init_window_persistence_once() -> bool:
    ...
    # Settings owns the account-scoped INI documents. ImGui owns window placement and collapse
    # state in the account's native imgui.ini; only non-geometry preferences use these documents.
    _window_ini_key = Settings(f'{WINDOW_INI_PATH}/{WINDOW_INI_FILE}', 'account').name
    _floating_ui_ini_key = Settings(f'{WINDOW_INI_PATH}/{FLOATING_UI_INI_FILE}', 'account').name
    _window_ini_ready = bool(_window_ini_key and _floating_ui_ini_key)
    return _window_ini_ready
...
def _draw_tab_bar() -> None:
    ...
    for key, label in [('teams', 'Teams'), ('templates', 'Templates')]:
        ...
        clicked = (
            _button_selected_tab(f'{label}##tab_{key}', 96, 24) if active else _button(f'{label}##tab_{key}', 96, 24)
        )
        if clicked:
            _active_tab = key
    PyImGui.separator()
...
def _load_template_filter_once() -> None:
    ...
    _only_show_compatible_templates = bool(
        Settings(_window_ini_key, 'account').get_bool(
...
def _save_template_filter_preference() -> None:
    if _init_window_persistence_once():
        Settings(_window_ini_key, 'account').set_bool(
            TEMPLATE_FILTER_SECTION,
            TEMPLATE_FILTER_KEY,
            _only_show_compatible_templates,
        )
...
def _template_options_for_slot(
    ...
    if selected_template_id and all(option_id != selected_template_id for option_id, _label, _template in options):
        ...
        if selected_template is not None:
            ...
            short_id = selected_template_id[:18]
            options.append((selected_template_id, f'<Missing template: {short_id}>', None))
    return options
    ...
    if _confirm_action == 'save_current_team':
        ...
        try:
            ...
            if count <= 0:
                ...
                save_account_config(config)
                _clear_account_dirty(f'Saved {count} current heroes to {saved_team.name}.')
        except Exception as exc:
            _status = f'Save current team failed: {exc}'
        return

    if _confirm_action == 'reload_shared_templates':
        _reload_shared_templates(discard_unsaved=True)
        return
...
def _confirm_action_severity() -> str:
    if _confirm_action in {'delete_team', 'delete_template'}:
        return 'error'
    if _confirm_action in {'save_changes', 'save_current_team', 'reload_shared_templates'}:
        return 'warning'
    return 'info'
...
def _draw_teams_tab(config: HeroTeamConfig) -> None:
    ...
    if rename_applied:
        ...
        try:
            save_account_config(config)
            _clear_rename()
            _clear_account_dirty('Team renamed and saved.')
        except Exception as exc:
...
def _draw_selected_template_details(config: HeroTeamConfig, template, running: bool) -> None:
    ...
    if rename_applied:
        ...
        try:
            save_template_name(template.template_id, template.name)
            _clear_rename()
            _mark_template_name_saved(config, template.template_id)
        except Exception as exc:
    ...
    hero_index = HERO_ID_TO_INDEX.get(template_preferred_hero_id(config, template.template_id), 0)
    if stacked:
        PyImGui.text('Preferred hero (this account)')
    try:
    ...
    try:
        hero_combo_label = '##template_hero' if stacked else 'Preferred hero (this account)##template_hero'
        selected_hero = _combo(hero_combo_label, hero_index, hero_labels(config))
    finally:
        _end_disabled(began_disabled)
    selected_hero_id = int(HERO_IDS[selected_hero])
    if not running and selected_hero_id != template_preferred_hero_id(config, template.template_id):
        set_template_preferred_hero_id(config, template.template_id, selected_hero_id)
        _mark_dirty('Template preferred hero changed. Click Save to persist.')
    _show_item_tooltip(
        'Team load is running.'
        if running
        else 'Account-specific preferred direct-apply target for this template; '
        'team assignment is still controlled in the Teams tab.'
    )
...
def _draw_template_browser_layout(config: HeroTeamConfig, running: bool) -> None:
    ...
    flags = int(
        PyImGui.TableFlags.BordersInnerV | PyImGui.TableFlags.SizingStretchProp | PyImGui.TableFlags.NoSavedSettings
    )
    try:
    ...
    if _button_colored_disabled('Delete##delete_template', running, 'error', 72, 24):
        if _selected_template_id:
            _request_confirmation(
                'delete_template',
                'Delete Template',
                'This shared template will be deleted globally for every account when you Save. '
                'Existing team assignments will remain as missing references until reassigned.',
...
def _draw_save_state_footer(config: HeroTeamConfig) -> None:
    ...
    if not dirty:
        save_label = 'Saved'
    elif account_dirty and global_dirty:
        save_label = 'Unsaved account + shared changes'
    elif global_dirty:
        save_label = 'Unsaved shared changes'
    else:
        save_label = 'Unsaved account changes'
    _colored_text(f'Save state: {save_label}', 'warning' if dirty else 'success')
    if running:
        _same_line(12)
    ...
    _same_line(8)
    if _button_colored_disabled('Reload Shared##reload_shared_templates', running, 'info', 128, 24):
        _reload_shared_templates()
    _show_item_tooltip(
        'Wait for team load to finish before reloading shared templates.'
        if running
        else 'Reload the global template library from disk. Unsaved shared edits will require confirmation.'
    )
