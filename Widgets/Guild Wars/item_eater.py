...
def _draw_quick_actions_tab() -> None:
    """Keep the original Item Eater one-click actions available in the merged UI."""

    global quantity_to_consume

    quantity_to_consume = PyImGui.input_int("Quantity##quick_actions_quantity", quantity_to_consume)
    PyImGui.separator()

    quick_actions = (
        ("Open Trick-or-Treat Bags", ModelID.Trick_Or_Treat_Bag.value),
        ("Eat Sugary Blue Drink", ModelID.Sugary_Blue_Drink.value),
        ("Eat Creme Brulee", ModelID.Creme_Brulee.value),
        ("Eat Fruitcake", ModelID.Fruitcake.value),
        ("Eat Eggnog", ModelID.Eggnog.value),
        ("Eat Snowman Summoner", ModelID.Snowman_Summoner.value),
        ("Eat Wintersday Gift", ModelID.Wintersday_Gift.value),
    )
    for label, model_id in quick_actions:
        if PyImGui.button("%s##quick_action_%s" % (label, model_id)):
            GLOBAL_CACHE.Coroutines.append(eat_items(model_id, quantity_to_consume))

    if PyImGui.button("Exchange Pumpkin Cookies for Pie##quick_action_pumpkin_pie"):
        GLOBAL_CACHE.Coroutines.append(exchange_pumpkins_for_pie(quantity_to_consume))
...
def main():
    ...
    if PyImGui.begin(MODULE_NAME, PyImGui.WindowFlags.AlwaysAutoResize):
        if PyImGui.begin_tab_bar("##ItemEaterTabs"):
            ...
            if PyImGui.begin_tab_item("Quick Actions"):
                _draw_quick_actions_tab()
