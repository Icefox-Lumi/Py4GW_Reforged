...
from Sources.frenkeyLib.item_mods_src.item_modifier_parser import ItemModifierParser
from Sources.frenkeyLib.item_mods_src.properties import ArmorProperty, AttributeRequirement, BowTypeProperty, DamageProperty, EnergyProperty, TargetItemTypeProperty
...
class ItemSnapshot:
    ...
    def _get_parsed_item_data(self) -> _LazyParsedItemData:
        if self._parsed_item_data is _UNSET:
            modifiers: list[ItemModifier] = Item.Mods.GetModifiers(self.id) if self.id > 0 and self.is_valid else []
    ...
    def name_enc(self) -> Optional[bytes]:
        if self._name_enc is _UNSET:
            self._name_enc = self._load_name_bytes(lambda item_id: PyItem(item_id).GetNameEnc())
    ...
    def singular_name_enc(self) -> Optional[bytes]:
        if self._singular_name is _UNSET:
            self._singular_name = self._load_name_bytes(lambda item_id: PyItem(item_id).GetSingleItemName())
    ...
    def complete_name_enc(self) -> Optional[bytes]:
        if self._complete_name_enc is _UNSET:
            self._complete_name_enc = self._load_name_bytes(lambda item_id: PyItem(item_id).GetCompleteNameEnc())
