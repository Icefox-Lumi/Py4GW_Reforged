...
from Py4GWCoreLib.HeroAI.cache_data import CacheData, INI_DIR
from Py4GWCoreLib.HeroAI.utils import IsHeroFlagged
from Py4GWCoreLib.HeroAI.constants import NUMBER_OF_SKILLS
from Py4GWCoreLib.HeroAI.commands import HeroAICommands
...
def is_account_outside_compass_range(account) -> bool:
    player_pos = Player.GetXY()
    account_pos = (account.AgentData.Pos.x, account.AgentData.Pos.y)
    return Utils.Distance(player_pos, account_pos) > Range.Compass.value + 10


def push_out_of_range_header_colors(style) -> None:
    style.Header.push_color((255, 0, 0, 170))
    style.HeaderHovered.push_color((255, 0, 0, 210))
    style.HeaderActive.push_color((255, 0, 0, 255))


def pop_out_of_range_header_colors(style) -> None:
    style.HeaderActive.pop_color()
    style.HeaderHovered.pop_color()
    style.Header.pop_color()


def push_out_of_range_window_style(style) -> None:
    style.WindowBg.push_color((90, 0, 10, 120))
    style.Border.push_color((90, 0, 10, 255))
    style.TitleBg.push_color((130, 0, 0, 255))
    style.TitleBgActive.push_color((200, 0, 0, 255))
    style.TitleBgCollapsed.push_color((130, 0, 0, 255))
    style.WindowBorderSize.push_style_var(3)


def pop_out_of_range_window_style(style) -> None:
    style.WindowBorderSize.pop_style_var()
    style.TitleBgCollapsed.pop_color()
    style.TitleBgActive.pop_color()
    style.TitleBg.pop_color()
    style.Border.pop_color()
    style.WindowBg.pop_color()


def push_compact_player_window_spacing(style) -> None:
    style.WindowPadding.push_style_var(2, 2)
    style.ItemSpacing.push_style_var(0, 0)
    style.ItemInnerSpacing.push_style_var(0, 0)


def pop_compact_player_window_spacing(style) -> None:
...
class HeroAI_PlayerPanelRenderer:
    male_aliases = [
        "Garrick Stonefaceplant",
        "Rurik Returned",
        "Talon Crittingham",
        "Brom Shieldslapper",
        "Dorian Interrupticus",
        "Kestrel Longrecharge",
        "Magnus Aftercastius",
        "Devlin Overpuller",
        "Harlan Aggrosworth",
        "Fenric Lineofsighter",
        "Alden Bodyblockson",
        "Torvin Energyless",
        "Cassian MissedAgain",
        "Perrin StillCasting",
        ...
        "Ulric Laghammer",
        "Bran OopsAllWipes",
        "Merek TooCloseToDoor",
        "Darion RezPlz",
        "Valric PullRangeTester",
    ]
    female_aliases = [
        "Lyessa Hexington",
        "Merali Spiritspammer",
        "Kaelira Wanderson",
        "Virelle Overaggra",
        "Elowyn TooSquishy",
        "Sarielle EnergyCrisis",
        "Nymera Castbarlong",
        "Tahlira Backlinerun",
        "Mireya Interruptia",
        "Kelindra PingTooHigh",
        "Vaelene LostCompass",
        "Ariselle Minionmancer",
        "Liorra WrongBuild",
        "Serethia StillLoading",
        "Ismara PetStuck",
        "Velina SpiritWall",
        "Caelyra Aftershockia",
        "Nyssara Overenchanted",
        "Talindra ShadowStepOops",
        "Melira FlagRunnerLost",
    ]
    ...
    def get_display_name(self, account) -> str:
        ...
        if not self.appearance_window.get_obfuscate_player_names():
            return name

        aliases = self.female_aliases if Agent.IsFemale(account.AgentData.AgentID) else self.male_aliases
        score_source = f"{name}|{account.AccountEmail}".encode("utf-8", errors="ignore")
        score = sum(score_source)
        return aliases[score % len(aliases)]
...
class HeroAI_RichPlayerPanelRenderer:
    ...
    def _draw_rich_skill_bar(self, account, options) -> None:
        ...
        for slot, skill_info in enumerate(account.AgentData.Skillbar.Skills):
            ...
            if not enough_adrenaline:
                ...
                PyImGui.draw_list_add_rect_filled(fill_rect[0], fill_rect[1], fill_rect[0] + fill_rect[2], fill_rect[1] + fill_rect[3], Color(0, 0, 0, 150).color_int, 0)
            ...
            if not options.Skills[slot]:
                if draw_textures:
                    ...
                    PyImGui.draw_list_add_rect_filled(item_rect_min[0], item_rect_min[1], item_rect_min[0] + skill_size, item_rect_min[1] + skill_size, Color(89, 0, 0, 120).color_int, 0)
    ...
    def _draw_buffs_and_upkeeps(self, account, skill_size: float = 28):
        ...
        def draw_buff(effect: CachedSkillInfo, duration: float, remaining: float, draw_effect_frame: bool = True, size: float = skill_size):
            ...
            if duration > 0 and remaining:
                ...
                PyImGui.draw_list_add_rect_filled(progress_background_rect[0], progress_background_rect[1], progress_background_rect[2], progress_background_rect[3], Color(0, 0, 0, 255).color_int, 0)
                ...
                PyImGui.draw_list_add_rect_filled(progress_rect[0], progress_rect[1], progress_rect[0] + progress_width, progress_rect[3], effect.progress_color, 0)
                ...
                PyImGui.draw_list_add_rect_filled(item_rect_min[0] + offset_x - 1, item_rect_min[1] + offset_y - 1, item_rect_min[0] + offset_x + text_size[0] + 1, item_rect_min[1] + offset_y + text_size[1] + 1, Color(0, 0, 0, 150).color_int, 2)
    ...
    def draw_panel(self, account, cached_data: CacheData, messages: list) -> None:
        ...
        for name, value in opt_dict.items():
            ...
            active = ImGui.toggle_button(name + f"##{account.AccountEmail}", value, control_row_width / len(opt_dict) - self._scale(3), control_height)
            ImGui.pop_font()
            if active != value:
...
class HeroAI_AppearanceWindow:
    ...
    def draw_window(self) -> None:
        ...
        expanded, open_ = ImGui.begin_with_close("HeroAI Appearance Settings", self.is_open(), PyImGui.WindowFlags.AlwaysAutoResize)
        self.set_open(open_)
        ...
        if expanded:
            ...
            if show_players:
                ...
                panel_mode = 1 if use_rich_player_panels else 0
                panel_mode = PyImGui.radio_button("Lite Panels", panel_mode, 0)
                PyImGui.same_line(0, -1)
                panel_mode = PyImGui.radio_button("Rich Panels", panel_mode, 1)
                new_use_rich_player_panels = panel_mode == 1
                if new_use_rich_player_panels != use_rich_player_panels:
                    self.set_use_rich_player_panels(new_use_rich_player_panels)

                if not new_use_rich_player_panels:
...
class HeroAI_MainWindow:
    ...
    def draw_window(self) -> None:
        expanded, open_ = ImGui.begin_with_close(HeroAIFloatingIcon.MODULE_NAME, self.floating_button.visible, PyImGui.WindowFlags.AlwaysAutoResize)
        ...
        if self.appearance_window.get_show_players() and not self.appearance_window.get_show_players_in_separate_window():
            ...
            if PyImGui.collapsing_header("Players##EmbeddedPlayers"):
                ...
                for account in sorted_accounts:
                    ...
                    display_name = self.player_renderer.get_window_title(account)
                    header_label = f"{player_index}. {display_name}##ControlPlayer{player_index}"
                    out_of_range = is_account_outside_compass_range(account)
                    if out_of_range:
                        push_out_of_range_header_colors(style)
                    header_open = PyImGui.collapsing_header(header_label)
                    if out_of_range:
                        pop_out_of_range_header_colors(style)
                    if header_open:
    ...
    def draw_window(self) -> None:
        ...
        if self.appearance_window.get_show_players_in_individual_windows():
            ...
            for account in player_accounts:
                ...
                style = ImGui.get_style()
                out_of_range = is_account_outside_compass_range(account)
                push_compact_player_window_spacing(style)
                if out_of_range:
                    push_out_of_range_window_style(style)
                window_open = PyImGui.begin(window_title, PyImGui.WindowFlags.AlwaysAutoResize)
                if out_of_range:
                    pop_out_of_range_window_style(style)
                if window_open:
                    if self.appearance_window.get_use_rich_player_panels():
                        self.player_renderer.draw_rich_panel(
                            account,
        ...
        expanded, open_ = ImGui.begin_with_close("HeroAI Players", self.appearance_window.get_show_players_in_separate_window(), PyImGui.WindowFlags.AlwaysAutoResize)
        ...
        if expanded:
            ...
            for account in player_accounts:
                ...
                out_of_range = is_account_outside_compass_range(account)
                if out_of_range:
                    push_out_of_range_header_colors(ImGui.get_style())
                header_open = PyImGui.collapsing_header(header_label)
                if out_of_range:
                    pop_out_of_range_header_colors(ImGui.get_style())
                if header_open:
                    if self.appearance_window.get_use_rich_player_panels():
                        self.player_renderer.draw_rich_panel(
                            account,
